...
function drawShapes(objects) {
	var i = 0, l = 0;
	for (i = 0, l = objects.length; i < l; i++)
		objects[i].obj.draw(context, objects[i].position, objects[i].pivot, objects[i].angle);
}
...
function drawSprites(objects, current, total) {
	...
	if (current >= total) {
		for (i = 0, l = objects.length; i < l; i++) {
			objects[i].obj.draw(context, 2, objects[i].position, objects[i].pivot, objects[i].angle);
			
			context.fillStyle = 'rgba(0, 0, 255, 1)';
			context.fillRect(objects[i].position.x(), objects[i].position.y(), 3, 3);
		}
	}
}
...
function drawTexts(objects) {
	var i = 0, l = 0;
	for (i = 0, l = objects.length; i < l; i++)
		objects[i].obj.draw(context, fonts[i], objects[i].position, objects[i].pivot, objects[i].angle);
}
